...
class FadeStickException(Exception):
    pass
...
def find_all():
    """
    Find all attached FadeStick devices.

    @rtype: FadeStick[]
    @return: a list of FadeStick objects or None if no devices found
    """
    results = []
    for d in usb.core.find(find_all=find_all, idVendor=VENDOR_ID, idProduct=PRODUCT_ID):
...
def find_by_serial(serial=None):
    """
    Find FadeStick device based on serial number.

    @rtype: FadeStick
    @return: FadeStick object or None if no devices are found
    """

    devices = []
    if sys.platform == "win32":
        devices = [d for d in findFadeSticks()
                   if d.serial_number == serial]
    else:
        for d in findFadeSticks():
            try:
...
class FadeStick(object):
    """
    FadeStick class is designed to control regular FadeStick devices
    """
